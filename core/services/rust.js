...
const runCode = (code, func) => {
  if (!isCommandAvailable("rustc")) {
    return func(
      missingRuntimeError(
        "Rust",
        "Install Rust and ensure the `rustc` command is available on PATH."
      )
    );
  }
      ...
      if (err) {
        ...
        fs.writeFile(path + fullName, code, function (err) {
          if (err) {
            ...
            const executable = process.platform === "win32" ? `${file}.exe` : file;
            const runCommand =
              process.platform === "win32" ? executable : `./${file}`;
            runShellCommand(
              {
                command: `rustc ${fullName} -o ${executable} && ${runCommand}`,
                cwd: folder,
                timeout: timeOut,
              },
